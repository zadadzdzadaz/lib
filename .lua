    ...
    -- Library element functions
        function library:window(properties)
            local cfg = { 
                ...
                game_name = properties.gameInfo or properties.game_info or properties.GameInfo or "by Y2K";
        ...
        -- ViewportFrame pour modèles 3D
        function library:viewport3d(options)
            local cfg = {
                name = options.name or "3D Model",
                model = options.model,
                camera_offset = options.camera_offset or cfr(0, 0, 5),
                rotation_speed = options.rotation_speed or 1,
                size = options.size or dim2(1, -8, 0, 200),
                items = {}
            }
        ...
        -- Viewport3D supprimé (obsolète/buggy)
        ...
        function library:progressbar(options)
            local cfg = {
                ...
                color = options.color or themes.preset.accent,
                items = {}
            }
            ...
            items["bar_bg"] = library:create("Frame", {
                Parent = items["container"],
                Position = dim2(0, 4, 0, 20),
                Size = dim2(1, -8, 0, 8),
                BackgroundColor3 = rgb(33, 33, 35),
            ...
            items["bar_fill"] = library:create("Frame", {
                Parent = items["bar_bg"],
                Size = dim2(0, 0, 1, 0),
            ...
            library:create("UICorner", {
                Parent = items["bar_fill"],
                CornerRadius = dim(0, 999)
            })
            
            items["percentage"] = library:create("TextLabel", {
                FontFace = fonts.small,
                TextColor3 = rgb(72, 72, 73),
                Text = "0%",
                Parent = items["container"],
            ...
            function cfg.set(value)
                ...
                library:tween(items["bar_fill"], {Size = dim2(percentage, 0, 1, 0)})
                items["percentage"].Text = string.format("%d%%", floor(percentage * 100))
            end
        ...
        function library:image(options)
            local cfg = {
                ...
                size = options.size or dim2(1, -8, 0, 150),
                items = {}
            }
            ...
            items["image"] = library:create("ImageLabel", {
                Parent = items["container"],
                Position = dim2(0, 4, 0, cfg.name ~= "" and 20 or 0),
                Size = cfg.size,
                Image = cfg.image,
                BackgroundColor3 = rgb(33, 33, 35),
                BorderSizePixel = 0,
            ...
            function cfg.set_image(image_id)
                items["image"].Image = image_id
            end
        ...
        function library:divider(options)
            ...
            if cfg.text then
                ...
                -- Lignes de chaque côté du texte
                library:create("Frame", {
                    Parent = items["container"],
        ...
        function library:range_slider(options)
            local cfg = {
                name = options.name or "Range",
                min = options.min or 0,
                max = options.max or 100,
                default_min = options.default_min or options.min or 0,
                default_max = options.default_max or options.max or 100,
                callback = options.callback or function() end,
                items = {}
            }
            ...
            items["slider_bg"] = library:create("Frame", {
                ...
                BackgroundColor3 = rgb(33, 33, 35),
                Size = dim2(1, -10, 0, 4),
                Position = dim2(0, 5, 0, 30),
                BorderSizePixel = 0
            })
            
            library:create("UICorner", {Parent = items["slider_bg"], CornerRadius = dim(0, 4)})
            ...
            items["value_label"] = library:create("TextLabel", {
                Parent = items["container"],
                Text = cfg.default_min .. " - " .. cfg.default_max,
            ...
            -- Note: Full slider logic would require input handling, 
            -- simplified here to show visual structure
            
            return setmetatable(cfg, library)
        end
        
        -- Gradient Picker
        function library:gradient_picker(options)
            local cfg = {
                name = options.name or "Gradient",
                color1 = options.color1 or rgb(255, 0, 0),
                color2 = options.color2 or rgb(0, 0, 255),
                callback = options.callback or function() end,
                items = {}
            }
            local items = cfg.items
            ...
            items["preview"] = library:create("Frame", {
                ...
                Position = dim2(1, -110, 0, 10),
                BorderSizePixel = 0,
                BackgroundColor3 = rgb(255, 255, 255)
            })
            
            library:create("UICorner", {Parent = items["preview"], CornerRadius = dim(0, 4)})
        ...
        -- Info Box
        function library:info_box(options)
            local cfg = {
                title = options.title or "Info",
                text = options.text or "",
                type = options.type or "info", -- info, warning, error, success
                items = {}
            }
            ...
            local colors = {
                ...
                error = rgb(255, 80, 80),
                success = rgb(50, 255, 100)
            }
            
            local accent = colors[cfg.type] or colors.info
            local items = cfg.items
            ...
            library:create("UICorner", {Parent = items["container"], CornerRadius = dim(0, 4)})
            library:create("UIStroke", {Parent = items["container"], Color = accent, Transparency = 0.5, Thickness = 1})
        ...
        function library:badge(options)
            ...
            -- Badges are typically attached to other elements, passing parent is tricky
            -- For simplicity, we create a small standalone badge
            ...
            local items = cfg.items
             ...
             items["badge_bg"] = library:create("Frame", {
                Parent = items["container"],
                BackgroundColor3 = cfg.color,
                Size = dim2(0, 0, 0, 18),
                AutomaticSize = Enum.AutomaticSize.X,
            ...
            library:create("UICorner", {Parent = items["badge_bg"], CornerRadius = dim(0, 4)})
            
            items["text"] = library:create("TextLabel", {
                Parent = items["badge_bg"],
                Text = cfg.text,
                TextColor3 = rgb(255, 255, 255),
                FontFace = fonts.bold,
                TextSize = 11,
        ...
        function library:toggle_group(options)
             local cfg = {
                name = options.name or "Mode",
                options = options.options or {"Option 1"},
            ...
            library:create("UIPadding", {Parent = items["title"], PaddingLeft = dim(0, 4)})
            
            local buttons_container = library:create("Frame", {
                Parent = items["container"],
                BackgroundTransparency = 1,
            ...
            local function update()
                 for opt, btn_objs in pairs(cfg.buttons) do
                    local is_selected = opt == cfg.default
                    
                    library:tween(btn_objs.bg, {BackgroundColor3 = is_selected and themes.preset.accent or rgb(30, 30, 35)})
                    library:tween(btn_objs.text, {TextColor3 = is_selected and rgb(255, 255, 255) or rgb(150, 150, 150)})
                end
            end
            ...
            for _, opt in ipairs(cfg.options) do
                ...
                library:create("UICorner", {Parent = btn_bg, CornerRadius = dim(0, 4)})
                ...
                cfg.buttons[opt] = {bg = btn_bg, text = btn_text}
